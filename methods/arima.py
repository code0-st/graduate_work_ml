...
def arima_predict(series, order=(1, 1, 1), forecast_steps=10, test_ratio=0.25, use_auto_arima=False):
...
def plot_forecast(result, title="Прогноз (ARIMA)"):
    train = result["train"]
...
def test_arima():
    ...
    result = arima_predict(series, order=(2, 1, 2), forecast_steps=15, use_auto_arima=False)
